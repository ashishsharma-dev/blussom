...
let dataTarget = document.querySelectorAll("[data-value='Beans']");

let data30package = dataTarget[0].dataset;
let data60package = dataTarget[1].dataset;
let data90package = dataTarget[2].dataset;

function days_30(type, itemNum) {
  // Set the data-target and data-value attributes based on the clicked product
  let dataTarget = itemNum;
  let dataValue = type === "beans" ? "Beans" : "Extract";

  // Log the data attributes for testing
  console.log("Data Target: " + dataTarget);
  console.log("Data Value: " + dataValue);

  // Get the select element
  let selectElement = document.getElementsByName("Subject")[0];

  // Find the option with the desired data attributes
  let desiredOption = selectElement.querySelector(
    'option[data-target="' + dataTarget + '"][data-value="' + dataValue + '"]'
  );
  ...
  if (desiredOption) {
    ...
    // Trigger a change event on the select element to notify any listeners
